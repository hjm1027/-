...
func AddCourse(c *gin.Context) {
	...
	for _, row := range rows {
		...
		onecourse := &model.UsingCourseModel{
			...
			Place1:   test(row[11]),
			Time2:    test(row[12]), //regexp.Match("^.{8}",row[12]),
			Place2:   test(row[13]),
			Time3:    test(row[14]), //regexp.Match("^.{8}",row[14]),
			Place3:   test(row[15]),
			Weeks1:   test(row[10]), //regexp.Match("$.{7}",row[10]),
			Weeks2:   test(row[12]), //regexp.Match("$.{7}",row[12]),
			Weeks3:   test(row[14]), //regexp.Match("$.{7}",row[14]),
